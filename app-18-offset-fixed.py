...
def _gld_proxy(gc_spot, authoritative_spot=None):
    ...
    for ticker_sym in GOLD_PROXIES:
        try:
            ...
            # Scan up to 6 expiries, score by OI concentration near spot (±20%).
            # Total OI is dominated by far-OTM strikes — near-spot OI is what matters.
            best_expiry, best_oi, best_calls, best_puts = None, 0, None, None
            scaled_spot = authoritative_spot if authoritative_spot else gc_spot
            for exp in exps[:6]:
                ...
                if ch.calls.empty or ch.puts.empty:
                    continue
                # Score = OI within ±20% of scaled spot only
                near_c = ch.calls[
                    (ch.calls["strike"] * scale).between(scaled_spot * 0.80, scaled_spot * 1.20)]
                near_p = ch.puts[
                    (ch.puts["strike"]  * scale).between(scaled_spot * 0.80, scaled_spot * 1.20)]
                near_oi = int(near_c["openInterest"].sum()) + int(near_p["openInterest"].sum())
                if near_oi > best_oi:
                    best_oi, best_expiry = near_oi, exp
...
def apply_dynamic_offset(level_dict, futures_spot, cfd_spot):
    """
    Dynamically translate futures-derived levels into CFD price space.
    offset = CFD_CURRENT - FUTURES_CURRENT
    adjusted_level = futures_level + offset
    """
    offset = cfd_spot - futures_spot

    adjusted = level_dict.copy()
    ...
    for key in level_keys:
        if key in adjusted:
            adjusted[key] = round(adjusted[key] + offset, 1)

    adjusted["Dynamic Offset"] = round(offset, 2)
    adjusted["Futures Spot"] = round(futures_spot, 2)
    adjusted["CFD Spot"] = round(cfd_spot, 2)
...
def fetch_all(_cache_buster: int = 0,
    ...
    # ── DYNAMIC FUTURES → CFD OFFSET NORMALIZATION ─────────────────────
    # We compute all dealer levels in native futures space first.
    # Then translate them into CFD space using:
    # offset = CFD_CURRENT - FUTURES_CURRENT
    # adjusted_level = futures_level + offset

    _gold_dte = dte(gc_expiry)

    # Native futures-space computation
    gold_df = enrich(gold_raw, gc_spot, GC_MULTIPLIER, _gold_dte)
    gold_atm_iv = get_atm_iv(gold_df, gc_spot)
    gold_lvl_native, gold_filt = levels(gold_df, gc_spot, "XAUUSD", days=_gold_dte)

    if xau_override > 1500:
        xau_display = xau_override

        # Dynamic live offset normalization
        gold_lvl = apply_dynamic_offset(
            gold_lvl_native,
            futures_spot=gc_spot,
            cfd_spot=xau_display
        )

        gold_source += (
            f"  |  ⚡ Dynamic CFD offset: "
            f"{gold_lvl['Dynamic Offset']:+.1f}"
        )

    else:
        xau_display = gc_spot
        gold_lvl = gold_lvl_native
...
def render_level_card(lvl):
    ...
    <div class="level-card">
      ...
      <div class="level-row"><span class="level-label">Dynamic Offset</span>
        <span class="level-val lvl-orange">{lvl.get('Dynamic Offset', 0):+,.1f}</span></div>
