...
def first_expiry(ticker_obj, ticker_symbol):
    ...
    # Prefer the 3rd available expiry when possible (typically better populated than the nearest),
    # otherwise fall back to the furthest available.
    return expiries[2] if len(expiries) >= 3 else expiries[-1]
...
def compute_0dte_walls(ticker_obj, ticker_symbol, display_label, display_spot, strike_scale):
    ...
    calls = calls[(calls["strike"] >= display_spot * 0.94) & (calls["strike"] <= display_spot * 1.06)].copy()
    puts = puts[(puts["strike"] >= display_spot * 0.94) & (puts["strike"] <= display_spot * 1.06)].copy()
    if calls.empty or puts.empty:
        return {
            "available": False,
            "reason": "0DTE chain has no strikes near live CFD spot after ETF-to-CFD conversion.",
            "expiry": today_str,
        }
...
def levels(df, spot, label="", days=30, iv_change=0.0, price_change=0.0,
    ...
    if filt is None or filt.empty:
        raise RuntimeError(
            "No option strikes are sufficiently close to the current spot. "
            "The option chain is not centred around the live CFD price."
        )

    near5 = filt[(filt["strike"] >= spot*0.95) & (filt["strike"] <= spot*1.05)]
    if len(near5) < 8:
        raise RuntimeError(
            f"Only {len(near5)} strikes are within ±5% of spot. "
            "Refusing to calculate unreliable GEX levels."
        )
