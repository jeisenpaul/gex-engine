...
def _gld_proxy(gc_spot):
    t        = yf.Ticker("GLD")
    gld_spot = float(t.history(period="5d")["Close"].dropna().iloc[-1])
    scale    = gc_spot / gld_spot
    best_expiry, best_oi, best_calls, best_puts = None, 0, None, None
    for exp in t.options[:4]:
        try:
            ch    = t.option_chain(exp)
            total = ch.calls["openInterest"].sum() + ch.puts["openInterest"].sum()
            if total > best_oi:
                best_oi, best_expiry, best_calls, best_puts = total, exp, ch.calls, ch.puts
        except: continue
    if best_calls is None:
        raise RuntimeError("GLD proxy: no data")
    df = pd.merge(
    ...
    return df, gc_spot, best_expiry, gld_spot, scale

# ── CORE COMPUTATION ──────────────────────────────────────────────────────────
def enrich(df, spot, multiplier, days=30):
    T  = max(days / 365.0, 1/365)
    df = df.copy()
    civ_l, piv_l, cg_l, pg_l = [], [], [], []
...
def backquant_block(label, lvl, spot, expiry):
    ...
    d0h  = round((spot + lvl["GEX Flip"])  / 2, 1)
    d0c  = round((spot + lvl["Call Wall"]) / 2, 1)
    d0p  = round((spot + lvl["Put Wall"])  / 2, 1)
    return (
        f"GEX Levels [{label}] - {now}\n"
        f"All-Expiry Levels:\n"
        ...
        f"0DTE Put: ${d0p:,.1f}\n"
        f"Advanced:\n"
        f"Zero Gamma: ${lvl['GEX Flip']:,.1f}\n"
        f"Max Pain: ${lvl['Max Pain']:,.1f}\n"
        f"Expected Move: ${lo:,.1f} to ${hi:,.1f}\n"
        f"Flip Zones (All): ${lvl['GEX Flip']:,.1f}\n"
    )
...
def fetch_all(xauusd_spot: float):
    ...
    if gold_raw is None:
        df_gld, gc_spot, gc_expiry, gld_spot, scale = _gld_proxy(gc_spot)
        gold_raw    = df_gld
        gold_source = f"GLD Proxy  (GLD ${gld_spot:.2f} × {scale:.2f})"
    ...
    s_lvl_sc  = {k: round(v*spx_scale,1) if k in ["Call Wall","Put Wall","GEX Flip","Max Pain"] else v
    ...
    q_lvl_sc  = {k: round(v*ndx_scale,1) if k in ["Call Wall","Put Wall","GEX Flip","Max Pain"] else v
