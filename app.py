...
def levels(df, spot, label="", days=30, iv_change=0.0, price_change=0.0, filter_pct=0.15):
    ...
    #
    # KEY INSIGHT from audit: negative GEX does NOT mean bearish.
    # Negative GEX = volatility expansion environment.
    # DIRECTION inside that environment is set by DEX + Vanna + price action.
    #
    # Hierarchy (highest priority first):
    #
    # TIER 1 — SQUEEZE OVERRIDE
    #   Triggered when: negative GEX + price rising + IV falling + Vanna bullish
    #   GEX says "vol expansion" but ALL other signals say "squeeze"
    #   → Classify as SQUEEZE BULLISH, not CRASH/TREND
    #
    # TIER 2 — CRASH OVERRIDE
    #   Triggered when: negative GEX + price falling + IV rising + Vanna bearish
    #   All signals aligned bearish — true trend/crash environment
    #   → Classify as CRASH / TREND SHORT
    #
    # TIER 3 — STANDARD GEX/DEX QUADRANTS
    #   Used when GEX is positive (pinning regime) or Vanna is mixed/neutral
    #   The four classic quadrants apply here
    #
    # Within each tier, DEX bias and Charm add texture to the strategy string.

    vanna_bullish = "BULLISH" in vanna_regime or "SQUEEZE" in vanna_regime
    vanna_bearish = "BEARISH" in vanna_regime and "BULLISH" not in vanna_regime

    squeeze_conditions = (
        not gex_positive   # negative GEX = vol expansion (directionally ambiguous)
        and price_up       # price moving up
        and iv_falling     # IV falling = squeeze fuel
        and vanna_bullish  # Vanna confirms bullish dealer flow
    )

    crash_conditions = (
        not gex_positive   # negative GEX = vol expansion
        and not price_up   # price moving down
        and not iv_falling # IV rising = crash fuel
        and vanna_bearish  # Vanna confirms bearish dealer flow
    )

    if squeeze_conditions:
        # TIER 1: Squeeze override — negative gamma but bullish Vanna + IV collapse
        # This is the "CRASH/TREND" label that was incorrectly assigned before
        if dex_positive:
            intraday_condition = "SQUEEZE TREND ▲"
            strategy           = "Momentum longs · Breakout bias · Vanna + DEX aligned · Trail stops up"
            condition_cls      = "regime-orange"
        else:
            intraday_condition = "SQUEEZE RISK ▲"
            strategy           = "Breakout watch · DEX lagging Vanna · Confirm reclaim of GEX flip"
            condition_cls      = "regime-orange"

    elif crash_conditions:
        # TIER 2: Crash/trend override — all signals aligned bearish
        if not dex_positive:
            intraday_condition = "CRASH / TREND ▼"
            strategy           = "Momentum shorts · Avoid catching bottoms · Trail stops down"
            condition_cls      = "regime-neg"
        else:
            intraday_condition = "BREAKDOWN RISK ▼"
            strategy           = "Short bias · DEX still positive but fading · Watch for capitulation"
            condition_cls      = "regime-neg"

    else:
        # TIER 3: Standard GEX/DEX quadrants (positive GEX pinning, or mixed Vanna)
        if gex_positive and dex_positive:
            ...
            # Negative GEX + DEX positive + Vanna not strongly aligned = ambiguous squeeze
            intraday_condition = "SQUEEZE WATCH"
            strategy           = "Momentum long bias · Confirm breakout · Vanna not fully aligned"
            condition_cls      = "regime-orange"
        else:
            # Negative GEX + DEX negative + no strong Vanna override = trend/range
            intraday_condition = "TREND / RANGE ▼"
            strategy           = "Short bias · Avoid breakout longs · Wait for Vanna confirmation"
            condition_cls      = "regime-neg"

    # ── Execution triggers ────────────────────────────────────────────────────
    # Conditional entry filters — tell you WHEN the regime is actionable.
    # Ordered from highest-conviction to mixed/waiting.
    if squeeze_conditions and dex_positive:
        entry_trigger = "⚡ SQUEEZE LONG: neg GEX + IV↓ + Vanna bullish + DEX confirmed"
        trigger_cls   = "regime-orange"
    elif squeeze_conditions and not dex_positive:
        entry_trigger = "⚡ SQUEEZE WATCH: neg GEX + IV↓ + Vanna bullish · wait for DEX flip"
        trigger_cls   = "regime-orange"
    elif crash_conditions and not dex_positive:
        entry_trigger = "✅ SHORT: neg GEX + IV↑ + Vanna bearish + DEX confirmed"
        trigger_cls   = "regime-neg"
    elif crash_conditions and dex_positive:
        entry_trigger = "⚠ BREAKDOWN RISK: neg GEX + IV↑ + Vanna bearish · DEX still holding"
        trigger_cls   = "regime-neg"
    elif gex_positive and dex_positive and price_up and iv_falling:
        entry_trigger = "✅ LONG: pos GEX + DEX confirmed · reclaim flip · IV falling"
        trigger_cls   = "regime-pos"
    elif gex_positive and not dex_positive and not price_up and not iv_falling:
        entry_trigger = "✅ SHORT: pos GEX + DEX bearish · below flip · IV rising"
        trigger_cls   = "regime-neg"
...
def fetch_all(_cache_buster: int = 0,
    ...
    if xau_override > 1500:
        ...
        gold_lvl, gold_filt = levels(gold_df, xau_display, "XAUUSD", days=_gold_dte, filter_pct=0.10)
        ...
        gold_lvl, gold_filt = levels(gold_df, gc_spot, "XAUUSD", days=_gold_dte, filter_pct=0.10)
