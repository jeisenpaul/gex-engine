...
def fetch_all(xauusd_spot: float):
    ...
    if gold_raw is None:
        # Pass xauusd_spot as authoritative so strikes scale to user's entered price
        df_gld, gc_spot, gc_expiry, etf_spot, scale, proxy_sym = _gld_proxy(gc_spot, authoritative_spot=xauusd_spot)
        gold_raw    = df_gld
    ...
    s_lvl_sc  = {k: round(v*spx_scale,1) if k in price_keys else v for k,v in s_lvl.items()}
    ...
    q_lvl_sc  = {k: round(v*ndx_scale,1) if k in price_keys else v for k,v in q_lvl.items()}
...
# ── RESULTS ───────────────────────────────────────────────────────────────────
if run:
    with st.spinner("Fetching data and computing GEX + Vanna + Charm levels…"):
        try:
