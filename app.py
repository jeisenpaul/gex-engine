...
if run:
    selected = []
    if us500_override > 1000:
        selected.append(("spy", "us500", "US500", us500_open))
    if us100_override > 5000:
        selected.append(("qqq", "us100", "US100", us100_open))
    if len(selected) == 0:
        st.error("Enter one CFD spot first: US500 or US100.")
        st.stop()
    if len(selected) > 1:
        st.error("Enter only one CFD spot at a time: US500 or US100.")
        st.stop()
