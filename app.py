...
def levels(df, spot, label="", days=30, iv_change=0.0, price_change=0.0,
    ...
    near_spot_count = int(((df["strike"] >= spot * 0.94) & (df["strike"] <= spot * 1.06)).sum())
    if near_spot_count == 0:
        strike_min = float(df["strike"].min())
        strike_max = float(df["strike"].max())
        raise ValueError(
            f"{label or 'Instrument'} option strikes do not cover live CFD spot {spot:,.1f}. "
            f"Converted strike range is {strike_min:,.1f}-{strike_max:,.1f}; refresh Yahoo data or verify the CFD/ETF mapping."
        )
