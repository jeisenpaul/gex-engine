...
def build_quick_read(lvl, odte, spot):
    """
 A five-line, plain-language summary for time-constrained execution:
 bias/confidence/regime, the single highest-dominance resistance and the
 single highest-dominance support (pooling BOTH the aggregate levels and
 the 0DTE walls together and ranking by dominance, so this is always "the
 one that actually matters" rather than an alphabetical list you have to
 re-rank yourself every time), what that dominance means given the
 current regime, and the plan. Meant to be read once, at a glance,
 instead of requiring a fresh manual pass through the full breakdown
 every session.
 """
    pool = []
    ...
    behavior = (
        "these are real inflection points to watch, not defended walls - "
        "negative gamma doesn't hold levels the way positive gamma does"
        if not gex_positive else
        "these are the levels most likely to actually cap/hold price (positive gamma pinning)"
    )

    lines = [
        f"QUICK READ: {lvl.get('Market State', 'n/a')} - {lvl.get('Directional Bias', 'n/a')} bias, "
        f"{lvl.get('Confidence', 'n/a')}% confidence ({lvl.get('Primary Regime', 'n/a')})",
    ]
    if resistances:
        r = resistances[0]
        lines.append(f"Resistance to watch: {r['name']} @ {r['level']:,.1f} (dominance {r['dominance_pct']}%)")
    else:
        lines.append("Resistance to watch: none above spot in this chain window")
    if supports:
        s = supports[0]
        lines.append(f"Support to watch: {s['name']} @ {s['level']:,.1f} (dominance {s['dominance_pct']}%)")
    else:
        lines.append("Support to watch: none below spot in this chain window")
