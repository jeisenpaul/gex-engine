...
def implied_vol(price, S, K, T, r, opt):
    intr = max(S-K, 0.0) if opt == "call" else max(K-S, 0.0)
    if price <= intr + 1e-6 or T <= 0:
        return 0.15
    try:
        fn = bs_call if opt == "call" else bs_put
        return brentq(lambda sig: fn(S, K, T, r, sig) - price, 1e-4, 5.0)
    except Exception:
        return 0.15
...
def _barchart_session(symbol="GLD"):
    from urllib.parse import quote, unquote
    s = requests.Session()
    s.headers.update(BARCHART_HEADERS)
    url = f"https://www.barchart.com/stocks/quotes/{quote(symbol, safe='')}/overview"
    s.get(url, timeout=20)
    xsrf = s.cookies.get("XSRF-TOKEN")
    if xsrf:
        s.headers.update({"x-xsrf-token": unquote(xsrf)})
    return s
...
def _barchart_quote(symbol, fallback=None):
    s = _barchart_session(symbol)
    r = s.get(
        "https://www.barchart.com/proxies/core-api/v1/quotes/get",
        params={"symbols": symbol, "fields": "symbol,lastPrice,previousPrice"},
        timeout=20,
    )
    r.raise_for_status()
    ...
    if not rows:
        if fallback is not None:
            return fallback
        raise RuntimeError(f"Barchart quote unavailable for {symbol}")
    price = _num(rows[0].get("lastPrice"), 0.0)
    if price <= 0:
        price = _num(rows[0].get("previousPrice"), 0.0)
    if price <= 0:
        if fallback is not None:
            return fallback
        raise RuntimeError(f"Barchart returned no usable price for {symbol}")
    return price


def _barchart_options(symbol, max_pages=4, limit=1000):
    s = _barchart_session(symbol)
    fields = ",".join([
        "symbol", "baseSymbol", "optionType", "strikePrice", "expirationDate",
        "volume", "openInterest", "lastPrice", "bidPrice", "askPrice", "impliedVolatility",
    ])
    records = []
    total = None
    for page in range(1, max_pages + 1):
        ...
        if r.status_code in (401, 403, 429):
            ...
            if r.status_code in (401, 403, 429):
                raise RuntimeError(f"Barchart blocked {symbol} options request: HTTP {r.status_code}")
        r.raise_for_status()
        payload = r.json()
    ...
    if not records:
        raise RuntimeError(f"No Barchart option rows returned for {symbol}")

    by_key = {}
    for row in records:
        raw = row.get("raw") if isinstance(row.get("raw"), dict) else row
        strike = _num(raw.get("strikePrice", row.get("strikePrice")), 0.0)
        if strike <= 0:
            continue
        expiry = raw.get("expirationDate") or row.get("expirationDate") or ""
        if "/" in str(expiry):
            try:
                expiry = datetime.strptime(str(expiry), "%m/%d/%y").strftime("%Y-%m-%d")
            except Exception:
                pass
        key = (float(strike), str(expiry))
        out = by_key.setdefault(key, {
            "strike": float(strike), "expiry": str(expiry),
            "call_oi": 0.0, "put_oi": 0.0,
            "call_iv": 0.0, "put_iv": 0.0,
            "call_settle": 0.0, "put_settle": 0.0,
        })
        side = str(raw.get("optionType", row.get("optionType", ""))).lower()
        oi = _num(raw.get("openInterest", row.get("openInterest")), 0.0)
        iv = _num(raw.get("impliedVolatility", row.get("impliedVolatility")), 0.0)
        if iv > 1.5:
            iv = iv / 100.0
        if iv <= 0.01:
            iv = 0.20
        last = _num(raw.get("lastPrice", row.get("lastPrice")), 0.0)
        if "call" in side:
            out["call_oi"] = oi
            out["call_iv"] = iv
            out["call_settle"] = last
        elif "put" in side:
            out["put_oi"] = oi
            out["put_iv"] = iv
            out["put_settle"] = last

    df_all = pd.DataFrame(by_key.values())
    df_all = df_all[(df_all["call_oi"] + df_all["put_oi"]) > 0]
    if df_all.empty:
        raise RuntimeError(f"Barchart returned {symbol} options, but open interest was empty")

    expiry_counts = df_all.groupby("expiry").size().sort_index()
    chosen_expiry = None
    ...
    if chosen_expiry is None:
        chosen_expiry = str(expiry_counts.index[0])

    df = (
        df_all[df_all["expiry"] == chosen_expiry]
        .drop(columns=["expiry"])
        .sort_values("strike")
        .reset_index(drop=True)
    )
    return df, chosen_expiry


def etf_to_cfd_price(etf_price, cfd_override=0.0, default_multiplier=1.0, min_override=0.0):
    """
    Convert ETF price into the CFD/index chart price.

    Examples:
    GLD 387 -> XAUUSD 3870 using multiplier 10
    SPY 746 -> US500 7460 using multiplier 10
    QQQ 740 -> US100 29600 using multiplier 40
    """
    if cfd_override and cfd_override > min_override:
        cfd_price = float(cfd_override)
    else:
        cfd_price = float(etf_price) * float(default_multiplier)
    scale = cfd_price / float(etf_price) if float(etf_price) > 0 else float(default_multiplier)
    return round(cfd_price, 1), round(scale, 4)


def etf_chain_to_cfd(df, scale):
    converted = df.copy()
    converted["strike"] = (converted["strike"] * scale).round(1)
    return converted


def _fetch_gold_spot(authoritative_spot=None):
    if authoritative_spot and authoritative_spot > 1500:
        return authoritative_spot, "user override"
    gld = _barchart_quote("GLD", fallback=388.0)  # GLD ~$385-395 as of June 2026
    spot, _ = etf_to_cfd_price(gld, default_multiplier=10.0)
    return spot, "Barchart GLD x10"


def _gld_proxy(gc_spot, authoritative_spot=None):
    spot_ref, spot_src = _fetch_gold_spot(authoritative_spot)
    gld_df, expiry = _barchart_options("GLD")
    gld_spot = spot_ref / 10.0
    _, scale = etf_to_cfd_price(gld_spot, cfd_override=spot_ref, default_multiplier=10.0, min_override=1500.0)
    df = etf_chain_to_cfd(gld_df, scale)
    label = f"BARCHART GLD OPTIONS ({spot_src})"
    return df, spot_ref, expiry, gld_spot, scale, label
...
def levels(df, spot, label="", days=30, iv_change=0.0, price_change=0.0):
    # ── Strike filter: ±15% of spot, hard cap ─────────────────────────────────
    # Document: "only keep strikes within ±15% of spot"
    # We try ±10% first for tighter levels, fall back to ±15% max.
    # We NEVER go beyond ±15% — far-OTM garbage pulls DEX flip and Vanna Flip
    # far outside the expected move zone and makes them meaningless.
    # ── Strike filter: hard cap ±12% for gold-proxy chains ───────────────────
    # GLD x10 scaling produces fine-grained ladders that look dense but include
    # far-OTM noise OI. Allowing ±20–25% breaks cumulative crossings → all flips
    # collapse to chain boundaries. Cap at ±12%; cascade 8→10→12→nearest-30.
    filt = None
    for pct in [0.08, 0.10, 0.12]:
    ...
    if filt is None or filt.empty:
        # Last resort: nearest 30 strikes to spot regardless of distance
        tmp = df.copy()
        tmp["_dist"] = (tmp["strike"] - spot).abs()
        filt = tmp.nsmallest(30, "_dist").drop("_dist", axis=1)\
                  .sort_values("strike").reset_index(drop=True)

    # ── GEX levels ────────────────────────────────────────────────────────────
    # Call wall: strike with peak positive call GEX (dealers most short calls here)
    call_wall = float(filt.loc[filt["call_gex"].idxmax(), "strike"])
    # Put wall: strike with peak absolute put GEX (put_gex is negative by convention)
    # Must use abs().idxmax() — idxmin() finds the most negative value which can
    # place put wall ABOVE call wall when OI is concentrated at high strikes.
    put_wall  = float(filt.loc[filt["put_gex"].abs().idxmax(), "strike"])

    cum = filt["net_gex"].cumsum().values
    crossings = np.where(np.diff(np.sign(cum)))[0]
    gex_distribution_warning = False
    if len(crossings) > 0:
    ...
    # Reject boundary pin: if flip is within 2% of wall edges, distribution is
    # monotonic/broken. Fall back to spot as the best neutral flip estimate.
    _wall_tol = max((hi_wall - lo_wall) * 0.02, 5.0)
    if abs(gflip - lo_wall) < _wall_tol or abs(gflip - hi_wall) < _wall_tol:
        gex_distribution_warning = True
        gflip = float(spot)
    ...
    _dwall_tol = max((hi_dwall - lo_dwall) * 0.02, 5.0)
    if abs(dflip - lo_dwall) < _dwall_tol or abs(dflip - hi_dwall) < _dwall_tol:
        dflip = float(spot)
    ...
    # Reject boundary pin on vanna flip too
    _vflip_lo = float(filt["strike"].min())
    _vflip_hi = float(filt["strike"].max())
    _vflip_tol = max((_vflip_hi - _vflip_lo) * 0.02, 5.0)
    if abs(vanna_flip - _vflip_lo) < _vflip_tol or abs(vanna_flip - _vflip_hi) < _vflip_tol:
        vanna_flip = float(spot)
    ...
    raw_max_pain = float(sa[np.argmin(lss)])
    # Clamp max pain between the walls, not the full filtered range.
    # Max pain above the Call Wall or below the Put Wall is a data artefact:
    # far-OTM OI from hedgers accumulates outside the tradeable band and pulls
    # the loss-minimisation result to the chain edge. The walls are the correct
    # structural boundaries for where max pain can realistically sit.
    _mp_lo = min(put_wall, call_wall)
    _mp_hi = max(put_wall, call_wall)
    max_pain = float(np.clip(raw_max_pain, _mp_lo, _mp_hi))
    ...
    return {
        ...
        "GEX Dist Warning":     gex_distribution_warning,
        "Trigger Class":        trigger_cls,
        # IV state
        "IV Change":            round(iv_change * 100, 2),   # in pct points
        "Price Change":         round(price_change, 2),
        "ATM IV":               round(get_atm_iv(filt, spot), 4),
    }, filt

def expected_move(spot, net_b, d=1, atm_iv=0.18):
    """
    Expected move using actual ATM IV from the chain, not a hardcoded estimate.
    d is capped at 1 for intraday expected move — what matters for trading is
    today's range, not the full expiry window which bloats the number uselessly.
    atm_iv: ATM implied volatility from get_atm_iv() — passed in from caller.
    """
    d    = max(int(d), 1)
    iv   = max(0.05, atm_iv)
    # Positive GEX = pinning → shrink by up to 15%
    iv   = iv * max(0.85, 1.0 - abs(net_b) * 0.02)
    # Intraday: use 1-day window (sqrt(1/252) not sqrt(d/365))
...
def backquant_block(label, lvl, spot, expiry):
    ...
    d    = dte(expiry)
    lo, hi = expected_move(spot, lvl["Net GEX $B"], d=1, atm_iv=lvl.get("ATM IV", 0.18))
    return (
...
def fetch_all(_cache_buster: int = 0,
    ...
    # ── GOLD ──────────────────────────────────────────────────────────────────
    # CME option data DISABLED: CME API consistently returns deferred-contract
    # strikes ($4,000–$4,700) because front-month GC options expire and CME
    # rolls to the next contract with carry premium. Cannot be reliably filtered
    # without knowing exact contract specs per expiry cycle.
    #
    # Architecture: synthetic chain always.
    # _gld_proxy fetches spot via XAU=X / GLD / IAU cross-validation (not GC=F),
    # then builds a realistic synthetic chain centered on that spot.
    # If user entered an override, that takes priority over all fetched prices.
    df_gld, gc_spot, gc_expiry, etf_spot, scale, proxy_sym = _gld_proxy(
        3880.0,   # gc_spot arg unused when authoritative_spot is valid; update if needed
        authoritative_spot=xau_override if xau_override > 1500 else None)
    gold_raw    = df_gld
    gold_source = proxy_sym

    # gold_raw is always set by _gld_proxy (synthetic chain) above.
    # No CME sanity guard needed — CME fetch is disabled.

    # If user provided a CFD override, the synthetic chain is already built
    # around that price (passed as authoritative_spot). No re-scaling needed.
    # Just use gc_spot (which equals xau_override when provided) as display spot.
    xau_display = gc_spot   # gc_spot = cross-validated real spot from _fetch_gold_spot

    _gold_dte   = dte(gc_expiry)
    gold_df     = enrich(gold_raw, xau_display, GC_MULTIPLIER, _gold_dte)
    ...
    # ── SPY / US500 ───────────────────────────────────────────────────────────
    s_spot = _barchart_quote("SPY")
    s_df, s_exp = _barchart_options("SPY")
    spx, _ = etf_to_cfd_price(s_spot, default_multiplier=10.0)
    # CFD override: user can correct auto-fetched ^GSPC if it's stale/wrong
    spx_display, spx_scale = etf_to_cfd_price(
        s_spot,
        cfd_override=us500_override,
        default_multiplier=10.0,
        min_override=1000.0,
    )

    # Scale SPY strikes to CFD display space before enriching — levels in CFD price space
    s_df_scaled = etf_chain_to_cfd(s_df, spx_scale)
    _s_dte   = dte(s_exp)
    ...
    # ── QQQ / US100 ───────────────────────────────────────────────────────────
    q_spot = _barchart_quote("QQQ")
    q_df, q_exp = _barchart_options("QQQ")
    ndx, _ = etf_to_cfd_price(q_spot, default_multiplier=40.0)
    # CFD override: user can correct auto-fetched ^NDX if it's stale/wrong
    ndx_display, ndx_scale = etf_to_cfd_price(
        q_spot,
        cfd_override=us100_override,
        default_multiplier=40.0,
        min_override=5000.0,
    )

    # Scale QQQ strikes to CFD display space before enriching — levels in CFD price space
    q_df_scaled = etf_chain_to_cfd(q_df, ndx_scale)
    _q_dte   = dte(q_exp)
...
def render_level_card(lvl):
    ...
    <div class="level-card">
      ...
      {'<div style="background:#3a1a00;border:1px solid #ff9800;border-radius:6px;padding:5px 10px;margin:4px 0;font-size:0.78rem;color:#ff9800;">&#9888; Dealer distribution unstable &mdash; GEX flip pinned to spot. Levels may be unreliable.</div>' if lvl.get("GEX Dist Warning") else ""}
...
if run:
    ...
    st.markdown("""
    <div class="footer">
      Levels cached 30m · Regime engine live (ATM IV · session-open anchor) · Barchart data<br>
