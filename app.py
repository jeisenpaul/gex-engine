...
def levels(df, spot, label="", days=30, iv_change=0.0, price_change=0.0):
    # Use ±15% window as the primary range (per audit recommendation).
    # Garbage far-OTM strikes inflate GEX Flip, DEX walls, and Vanna — cap them.
    ...
    # ── GEX levels ────────────────────────────────────────────────────────────
    # Call Wall = strike with highest positive call GEX (above spot — resistance)
    # Put Wall  = strike with most negative put GEX (below spot — support)
    # MUST use separate call_gex / put_gex columns — combined net_gex can
    # invert the walls because put_gex is already negated in enrich().
