...
# ── SESSION HELPERS ───────────────────────────────────────────────────────────
def get_iv_state(current_iv, key):
    if key not in st.session_state:
        st.session_state[key] = current_iv

    opening_iv = st.session_state[key]
...
def enrich(df, spot, multiplier, days=30):
    ...
    # CEX (Charm Exposure): charm × OI × multiplier × S  (per year → /365 for per day)
    # Positive net CEX = dealers buy delta each day (bullish daily bleed)
...
def levels(df, spot, label="", current_iv=0.0, expiry_days=0, current_iv=float(filt["call_iv"].mean()) if "call_iv" in filt.columns else 0.0, expiry_days=3):
    ...
    dex_bias = "BULLISH ▲"  if total_net_dex > 0 else "BEARISH ▼"

    # Vanna regime: net VEX sign tells you dealer response to vol moves
    # Positive: vol drop → dealers buy spot (supportive of rallies in low-vol env)
    # Negative: vol drop → dealers sell spot (suppressive; watch for vol spikes)
    vanna_regime = "VOL-SPIKE BULLISH ▲" if total_net_vex > 0 else "VOL-SPIKE BEARISH ▼"

    # Charm regime: positive = daily delta bleed is bullish (dealers accumulate)
    # Most relevant in last 5 days before expiry
    charm_flow = "BULLISH BLEED ▲" if total_net_cex > 0 else "BEARISH BLEED ▼"

    # ── IV Tracking ──────────────────────────────────────────────────────────
    iv_key = f"{label}_open_iv"
    iv_state, iv_change = get_iv_state(current_iv, iv_key)

    # ── Dynamic Vanna Flow ───────────────────────────────────────────────────
    if total_net_vex > 0 and iv_change < 0:
        vanna_signal = "BULLISH SQUEEZE ▲"
    elif total_net_vex < 0 and iv_change > 0:
        vanna_signal = "BEARISH ACCELERATION ▼"
    else:
        vanna_signal = "NEUTRAL FLOW ▬"

    # ── Charm Timing ────────────────────────────────────────────────────────
    if expiry_days <= 1:
        expiry_pressure = "EXTREME"
    elif expiry_days <= 5:
        expiry_pressure = "HIGH"
    else:
        expiry_pressure = "LOW"

    current_hour = datetime.now().hour

    if current_hour >= 13 and total_net_cex > 0:
        charm_signal = "AFTERNOON BUY FLOW ▲"
    elif current_hour >= 13 and total_net_cex < 0:
        charm_signal = "AFTERNOON SELL FLOW ▼"
    else:
        charm_signal = "NEUTRAL CHARM ▬"

    # ── Intraday Trade Engine ───────────────────────────────────────────────
    trade_condition = "NEUTRAL ▬"
    preferred_trade = "WAIT"
    ...
    if (
        ...
        trade_condition = "SQUEEZE ENVIRONMENT ▲"
        preferred_trade = "BUY PULLBACKS"
        ...
        trade_condition = "LIQUIDATION ENVIRONMENT ▼"
        preferred_trade = "SELL RALLIES"

    elif (
        total_net_gex > 0 and
        abs(total_net_dex) < abs(total_net_gex) * 0.5 and
        abs(iv_change) < 0.01
    ):
        trade_condition = "PINNED RANGE ▬"
        preferred_trade = "MEAN REVERSION"
        ...
        trade_condition = "STABLE BULL TREND ▲"
        preferred_trade = "BUY DIPS"
    ...
    return {
        ...
        "IV State":         iv_state,
        "IV Change":        round(iv_change, 4),
        "Vanna Signal":     vanna_signal,
        "Charm Signal":     charm_signal,
        "Expiry Pressure":  expiry_pressure,
        "Trade Condition":  trade_condition,
        "Preferred Trade":  preferred_trade,
    }, filt
...
def fetch_all(_cache_buster: int = 0,
    ...
    if xau_override > 1500:
        ...
        gold_df = enrich(gold_raw_display, xau_display, GC_MULTIPLIER, dte(gc_expiry))
        gold_lvl, gold_filt = levels(gold_df, xau_display, "XAUUSD", current_iv=float(filt["call_iv"].mean()) if "call_iv" in filt.columns else 0.0, expiry_days=3)
        ...
        gold_lvl, gold_filt = levels(gold_df, gc_spot, "XAUUSD", current_iv=float(filt["call_iv"].mean()) if "call_iv" in filt.columns else 0.0, expiry_days=3)
    ...
    s_rich   = enrich(s_df_scaled, spx_display, EQ_MULTIPLIER, dte(s_exp))
    s_lvl, s_filt = levels(s_rich, spx_display, "US500", current_iv=float(filt["call_iv"].mean()) if "call_iv" in filt.columns else 0.0, expiry_days=3)
    ...
    q_rich   = enrich(q_df_scaled, ndx_display, EQ_MULTIPLIER, dte(q_exp))
    q_lvl, q_filt = levels(q_rich, ndx_display, "US100", current_iv=float(filt["call_iv"].mean()) if "call_iv" in filt.columns else 0.0, expiry_days=3)
...
def render_level_card(lvl):
    ...
    # Net CEX is per-day delta units — format compactly
