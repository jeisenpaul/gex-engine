...
def chain_covers_spot(df, spot, pct=0.12, min_rows=8):
    """True when the option chain has enough strikes around the current spot."""
    if df is None or df.empty or spot <= 0 or "strike" not in df.columns:
        return False
    strikes = pd.to_numeric(df["strike"], errors="coerce").dropna()
    if strikes.empty:
        return False
    near = strikes[(strikes >= spot * (1 - pct)) & (strikes <= spot * (1 + pct))]
    brackets_spot = strikes.min() <= spot <= strikes.max()
    return brackets_spot and len(near) >= min_rows
...
def levels(df, spot, label="", days=30, iv_change=0.0, price_change=0.0):
    ...
    def side_level(column, side, mode):
        """Pick resistance from strikes above spot and support from strikes below spot."""
        side_df = filt[filt["strike"] >= spot] if side == "above" else filt[filt["strike"] <= spot]
        if side_df.empty or side_df[column].abs().sum() == 0:
            side_df = filt
        idx = side_df[column].idxmax() if mode == "max" else side_df[column].idxmin()
        return float(side_df.loc[idx, "strike"])

    # ── GEX levels ────────────────────────────────────────────────────────────
    call_wall = side_level("call_gex", "above", "max")
    put_wall  = side_level("put_gex",  "below", "min")
    ...
    # ── DEX levels ────────────────────────────────────────────────────────────
    call_delta_wall = side_level("call_dex", "above", "max")
    put_delta_wall  = side_level("put_dex",  "below", "min")
...
def fetch_all(_cache_buster: int = 0,
    ...
    if gold_raw is None:
        ...
        GOLD_HARD_MAX = anchor * 1.25 if anchor > 1500 else float("inf")
        GOLD_HARD_MIN = anchor * 0.75 if anchor > 1500 else 0.0
        ...
        too_high = False
        far_from_anchor = anchor_ok and not chain_covers_spot(gold_raw, anchor, pct=0.12, min_rows=8)
