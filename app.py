...
def build_scaled_index(symbol, index_symbol, fallback_scale, override, override_min,
    ...
    try:
        ...
        auto_converted_spot = etf_spot * fallback_scale
        conversion_source = "ETF_FALLBACK"

    display_spot = float(override) if override > override_min else float(auto_converted_spot)
    if override > override_min:
    ...
    return {
        ...
        "auto_index": float(auto_converted_spot),
...
<div style="background:#0d1117;border:1px solid #1a2a3a;border-radius:8px;padding:10px 16px 6px 16px;
  ...
  CFD SPOT OVERRIDES &nbsp;-&nbsp; leave at 0 to use converted ETF prices only
</div>
""", unsafe_allow_html=True)
