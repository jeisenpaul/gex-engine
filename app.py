...
def assess_level_quality(filt, spot, level_values):
    ...
    if total_oi <= 0 or unique_strikes < 8 or max_level_cluster >= 6:
        confidence = "LOW"
...
def levels(df, spot, label="", days=30, iv_change=0.0, price_change=0.0,
    ...
    df = df[(df["strike"] > 0) & ((df["call_oi"] > 0) | (df["put_oi"] > 0))]
    df = df.sort_values("strike").reset_index(drop=True)
    if df.empty:
        raise ValueError(f"No strikes with usable open interest available for {label or 'instrument'}.")
    ...
    regime = "POSITIVE UP" if gex_positive else "NEGATIVE DOWN"
...
<div style="background:#0d1117;border:1px solid #1a2a3a;border-radius:8px;padding:10px 16px 6px 16px;
  ...
  CFD SPOT OVERRIDES &nbsp;-&nbsp; leave at 0 to use auto-fetched prices (^GSPC / ^NDX)
</div>
""", unsafe_allow_html=True)
...
if run:
    ...
    with tab1:
        ...
        st.markdown(f'<span class="source-badge badge-src">US500 {s["index_spot"]:.1f}{override_tag} (SPY x {s["scale"]:.4f}){auto_note}</span>', unsafe_allow_html=True)
    ...
    with tab2:
        ...
        st.markdown(f'<span class="source-badge badge-src">US100 {q["index_spot"]:.1f}{override_tag} (QQQ x {q["scale"]:.4f}){auto_note}</span>', unsafe_allow_html=True)
