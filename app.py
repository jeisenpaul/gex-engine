...
def fetch_all(_cache_buster: int = 0):
    ...
    # Scale SPY strikes to SPX before enriching — levels computed in SPX price space
    s_df_scaled = s_df.copy()
    s_df_scaled["strike"] = (s_df_scaled["strike"] * spx_scale).round(1)
    s_rich   = enrich(s_df_scaled, spx, EQ_MULTIPLIER, dte(s_exp))
    s_lvl, s_filt = levels(s_rich, spx, "US500")
    # No post-hoc scaling needed — levels already in SPX space
    price_keys = ["Call Wall","Put Wall","GEX Flip","Call DEX Wall","Put DEX Wall","DEX Flip","Max Pain","Vanna Flip"]
    s_lvl_sc  = s_lvl   # already scaled
    ...
    # Scale QQQ strikes to NDX before enriching — levels computed in NDX price space
    q_df_scaled = q_df.copy()
    q_df_scaled["strike"] = (q_df_scaled["strike"] * ndx_scale).round(1)
    q_rich   = enrich(q_df_scaled, ndx, EQ_MULTIPLIER, dte(q_exp))
    q_lvl, q_filt = levels(q_rich, ndx, "US100")
    # No post-hoc scaling needed — levels already in NDX space
    q_lvl_sc  = q_lvl   # already scaled
...
with col1:
    st.markdown("""
    <div style="background:#0d1f2d;border:1px solid #1a3050;border-radius:8px;padding:12px 16px;font-family:'JetBrains Mono',monospace;font-size:0.82rem;color:#8a9bb0;">
        Spots fetched automatically · XAU=X (interbank) · ^GSPC · ^NDX
    </div>
    """, unsafe_allow_html=True)
with col2:
...
with col_cc:
    if st.button("🗑 Clear Cache"):
        fetch_all.clear()
        st.success("Cache cleared — click Generate Levels")
        st.rerun()
...
if run:
    ...
    with tab2:
        s = data["spy"]
        st.markdown(f'<span class="source-badge badge-cme">SPX {s["index_spot"]:.1f}  (via SPY×{s["scale"]:.4f})</span>',
                    unsafe_allow_html=True)
    ...
    with tab3:
        q = data["qqq"]
        st.markdown(f'<span class="source-badge badge-cme">NDX {q["index_spot"]:.1f}  (via QQQ×{q["scale"]:.4f})</span>',
                    unsafe_allow_html=True)
