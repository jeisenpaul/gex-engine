...
def score_level_strength(by_strike, spot, level_map, gex_positive, dex_positive, pain_curve=None):
 ...
 """
    ...
    for name, level in levels_f.items():
        ...
        # Dominance means something different depending on regime, and that
        # difference is exactly where confusion happens in practice: high
        # dominance reads as "strong defended wall" in POSITIVE GAMMA, but
        # in NEGATIVE GAMMA there mostly isn't a defending force at all -
        # a high-dominance level there is a real INFLECTION POINT to watch,
        # not a floor/ceiling to expect a bounce from. Flag it explicitly
        # rather than leaving that reinterpretation implicit, especially
        # when the wall coincides with GEX Flip (the actual regime
        # boundary) or HVL (the single largest gamma concentration in the
        # whole chain) - those coincidences are the highest-value case to
        # call out because the label ("Call Wall", "Put Wall") undersells
        # what the level structurally is.
        regime_note = None
        if family in ("call_gex_wall", "put_gex_wall", "call_dex_wall", "put_dex_wall"):
            gex_flip_level = levels_f.get("GEX Flip")
            hvl_level = levels_f.get("HVL")
            coincides_flip = gex_flip_level is not None and abs(level - gex_flip_level) <= max(spot * 0.001, 2.0)
            coincides_hvl = hvl_level is not None and abs(level - hvl_level) <= max(spot * 0.001, 2.0)
            if coincides_flip:
                regime_note = (
                    "this strike IS the GEX Flip - the regime boundary itself, not just a wall. "
                    + ("A reclaim/hold here would flip NEGATIVE -> POSITIVE gamma."
                       if not gex_positive else
                       "A break/close through here would flip POSITIVE -> NEGATIVE gamma.")
                )
            elif coincides_hvl:
                regime_note = "this strike IS the HVL - the single largest gamma concentration in the whole chain, not just a side-specific wall."
            elif not gex_positive:
                regime_note = (
                    "in NEGATIVE GAMMA, high dominance here means real inflection point to watch, "
                    "not a defended wall to expect a bounce from - that defending force is a "
                    "positive-gamma behaviour this regime doesn't have."
                )
...
def build_quick_read(lvl, odte, spot):
    """
 A compact, plain-language summary for time-constrained execution:
 bias/confidence/regime, an honest retracement-risk read (negative gamma
 is the real structural driver of chop before a move confirms - this is
 a TILT based on regime/vanna, not a timestamped forecast of exactly when
 a retracement happens), the primary target level, the pullback/bounce
 zone ranked by dominance, and the plan. Pools BOTH the aggregate levels
 and the 0DTE walls together and ranks by dominance, so this is always
 "the one that actually matters" rather than a list to re-rank by hand.

 On dominance and the "it held anyway" case: dominance is a real,
 useful tilt, but it's a single-expiry, single-side OI concentration
 metric - it is NOT the only reason a level holds. Max Pain gravity and
 the Expected Move band are separate forces this number doesn't capture
 at all, so a level can hold on one of those even when its own dominance
 reads low. Rather than pretend dominance alone explains every hold,
 this checks the bounce zone against Max Pain / Expected Move directly
 and only raises the "this number is modest" caveat when neither of
 those other forces is present either - so the caveat means something
 when it does appear, instead of being boilerplate on every read.
 """
    pool = []
    for item in lvl.get("Level Strength", []):
        pool.append({"name": item["name"], "level": item["level"], "dominance_pct": item["dominance_pct"]})
    if odte and odte.get("available"):
        for item in odte.get("strength", []):
            pool.append({"name": item["name"], "level": item["level"], "dominance_pct": item["dominance_pct"]})
    if not pool:
        return "QUICK READ: not enough level data to summarise yet."
    ...
    # OI concentration points). Max Pain is the classic "gravitational
    # pull toward a settlement price" concept; when HVL (the single
    # largest gamma concentration) coincides with it, that pull is
    # reinforced rather than being two separate, weaker claims. Strength
    # depends on regime AND time-to-expiry: positive gamma pinning is a
    # real, actively-defended pull (dealers trade to keep price near their
    # biggest hedge concentration); negative gamma has no such defending
    # force, so the pull there is only as real as classic pin-risk
    # dynamics close to expiry - genuinely weak further out.
    ...
    # defending hedging force positive gamma has. A magnet pulling against
    # the current bias is a second, independent reason chop can show up
    # before the move confirms - this is a regime-level TILT built from
    # however many of these factors are actually present, not a
    # prediction of the exact moment a retracement happens.
    if gex_positive:
        retrace_tier = "NORMAL"
        ...
        if len(reasons) >= 2:
            retrace_tier = "HIGH"
            outcome = "real chop likely before the move confirms"
        elif len(reasons) == 1:
            retrace_tier = "ELEVATED"
            outcome = "some chop possible before confirmation"
        else:
            retrace_tier = "MODERATE"
            outcome = "this regime still doesn't defend levels the way positive gamma does"
        reason_text = " + ".join(reasons) if reasons else "session has settled, vanna isn't elevated, and the magnet isn't opposing the move"
        retrace_note = f"negative gamma with {reason_text} - {outcome}"

    # --- Primary target vs. pullback bounce zone: whichever side is IN the
    # direction of the bias is the target; the OPPOSITE side is where a
    # retracement would most likely find a reaction, ranked by dominance.
    if bias == "BULLISH":
        primary_label, primary_list = "Resistance to watch", resistances
        bounce_label, bounce_list = "Support / pullback bounce zone", supports
    else:
        primary_label, primary_list = "Support to watch", supports
        bounce_label, bounce_list = "Resistance / pullback bounce zone", resistances

    lines = [
        f"QUICK READ: {lvl.get('Market State', 'n/a')} - {bias} bias, "
        f"{lvl.get('Confidence', 'n/a')}% confidence ({lvl.get('Primary Regime', 'n/a')})",
        f"Retracement Risk: {retrace_tier} - {retrace_note}",
    ]
    ...
    if primary_list:
        p = primary_list[0]
        lines.append(f"{primary_label}: {p['name']} @ {p['level']:,.1f} (dominance {p['dominance_pct']}%)")
    else:
        lines.append(f"{primary_label}: none in this chain window")

    if bounce_list:
        b = bounce_list[0]
        max_pain = lvl.get("Max Pain")
        em_lo, em_hi = lvl.get("Expected Move Lo"), lvl.get("Expected Move Hi")
        tol = max(spot * 0.0015, 3.0)
        confluences = []
        if max_pain is not None and abs(b["level"] - max_pain) <= tol:
            confluences.append("Max Pain")
        if em_lo is not None and abs(b["level"] - em_lo) <= tol:
            confluences.append("Expected Move Lower")
        if em_hi is not None and abs(b["level"] - em_hi) <= tol:
            confluences.append("Expected Move Upper")
        conf_text = f" - also aligns with {', '.join(confluences)}" if confluences else ""
        lines.append(f"{bounce_label}: {b['name']} @ {b['level']:,.1f} (dominance {b['dominance_pct']}%{conf_text})")
        if b["dominance_pct"] < 25 and not confluences:
            lines.append(
                "Dominance here is modest with no Max Pain/Expected Move backup either - "
                "a hold is possible but not well-supported by this data; treat it as the "
                "least reliable level on this board, not a confirmed bounce zone"
            )
    else:
        lines.append(f"{bounce_label}: none in this chain window")
...
def backquant_block(label, lvl, spot, expiry, odte=None, ndx_check=None):
    ...
    strength_lines = "\n".join(
        ...
        + (f"\n   -> {item['regime_note']}" if item.get("regime_note") else "")
        for item in lvl.get("Level Strength", [])
    )
    if odte and odte.get("available"):
        odte_strength_lines = "\n".join(
...
def render_level_card(lvl, spot, odte=None, ndx_check=None):
 ...
 {f'<div style="font-size:0.68rem;color:#ffd740;font-weight:normal;margin-top:2px;max-width:220px;">{escape(item["regime_note"])}</div>' if item.get('regime_note') else ''}
 </span></div>"""
        for item in lvl.get("Level Strength", [])
    )
    flow = lvl.get("Flow Confidence", {})
