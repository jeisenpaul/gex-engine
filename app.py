...
def build_scaled_index(symbol, index_symbol, fallback_scale, override, override_min,
    ...
    block = backquant_block(display_label, lvl, display_spot, expiry, odte=odte)
...
def build_quick_read(lvl, odte, spot):
 ...
 """
    ...
    # defending hedging force positive gamma has. This is a regime-level
    # TILT, not a prediction of the exact moment a retracement happens.
    if gex_positive:
        retrace_tier = "NORMAL"
        ...
        if early_session and high_vanna:
            retrace_tier = "HIGH"
            retrace_note = "negative gamma + early session + elevated vanna fuel - real chop likely before the move confirms"
        elif early_session or high_vanna:
            reason = "early-session volatility" if early_session else "elevated vanna fuel"
            retrace_tier = "ELEVATED"
            retrace_note = f"negative gamma with {reason} - some chop possible before confirmation"
        else:
            retrace_tier = "MODERATE"
            retrace_note = "negative gamma alone - session has settled and vanna isn't especially elevated, but this regime still doesn't defend levels the way positive gamma does"
...
def backquant_block(label, lvl, spot, expiry, odte=None):
...
us100_override: float = 0.0):
    ...
    return results
    # -- LEVEL CARD RENDERER -------------------------------------------------------
def render_level_card(lvl, spot, odte=None):
...
if run:
    ...
    odte = item.get("odte")
    render_level_card(lvl, item["index_spot"], odte)
    st.markdown("**BackQuant Paste Block:**")
    st.code(backquant_block(label, lvl, item["index_spot"], item["expiry"], odte=odte), language=None)
