...
def scale_chain_to_display(raw_df, display_spot, source_spot, fallback_scale):
    """Convert ETF option strikes into broker CFD space and guard stale chain/spot ratios."""
    if display_spot <= 0 or source_spot <= 0:
        raise RuntimeError("Invalid spot data for CFD strike conversion.")

    def apply_scale(scale):
        scaled = raw_df.copy()
        scaled["strike"] = (scaled["strike"] * scale).round(1)
        return scaled

    def near_count(df, pct=0.05):
        return int(((df["strike"] >= display_spot * (1 - pct)) &
                    (df["strike"] <= display_spot * (1 + pct))).sum())

    base_scale = display_spot / source_spot
    scaled_df = apply_scale(base_scale)
    if near_count(scaled_df) >= 4:
        return scaled_df, base_scale, "CFD_OVERRIDE"

    raw = raw_df.copy()
    raw["total_oi"] = pd.to_numeric(raw.get("call_oi", 0), errors="coerce").fillna(0.0)
    raw["total_oi"] += pd.to_numeric(raw.get("put_oi", 0), errors="coerce").fillna(0.0)
    liquid = raw[raw["total_oi"] > 0].copy()
    if liquid.empty:
        liquid = raw.copy()
        liquid["total_oi"] = 1.0

    # When Yahoo's ETF spot and returned option chain are out of sync, anchor the
    # highest-liquidity strike to the live CFD spot instead of printing unusable walls.
    anchor_idx = liquid["total_oi"].idxmax()
    anchor = float(liquid.loc[anchor_idx, "strike"])
    rebase_scale = display_spot / anchor if anchor > 0 else fallback_scale
    rebase_df = apply_scale(rebase_scale)
    if near_count(rebase_df) >= near_count(scaled_df):
        return rebase_df, rebase_scale, "CFD_REBASED_CHAIN"

    return scaled_df, base_scale, "CFD_OVERRIDE_LOW_COVERAGE"
...
def build_scaled_index(symbol, index_symbol, fallback_scale, override, override_min,
    ...
    scaled_df, scale, conversion_source = scale_chain_to_display(
        raw_df, display_spot, etf_spot, fallback_scale
    )
    days = dte(expiry)
    ...
    odte = compute_0dte_walls(ticker, symbol, display_label, display_spot, scale)
...
def time_to_0dte_expiry_years():
    now_ny = market_now("America/New_York")
    market_close = now_ny.replace(hour=16, minute=0, second=0, microsecond=0)
    hours_remaining = max((market_close - now_ny).total_seconds() / 3600.0, 0.01)
    return hours_remaining / (365.0 * 6.5), hours_remaining, now_ny.strftime("%Y-%m-%d")


def _clean_0dte_side(df):
    needed = ["strike", "openInterest", "impliedVolatility"]
    ...
    clean = clean[(clean["strike"] > 0) & (clean["openInterest"] > 0)].copy()
    return clean[needed].sort_values("strike").reset_index(drop=True)


def get_0dte_atm_iv(calls_0dte, puts_0dte, spot):
    ivs = []
    for side in (calls_0dte, puts_0dte):
        if side is None or side.empty:
            continue
        clean = side[(side["impliedVolatility"] > 0.01) & (side["strike"] > 0)].copy()
        if clean.empty:
            continue
        idx = (clean["strike"] - spot).abs().idxmin()
        ivs.append(float(clean.loc[idx, "impliedVolatility"]))
    return float(np.mean(ivs)) if ivs else 0.20


def score_0dte_wall_strength(by_strike, wall, side, spot, hours_remaining):
    side_col = "call_gex" if side == "call" else "put_gex"
    oi_col = "call_oi" if side == "call" else "put_oi"
    side_abs = by_strike[side_col].abs()
    total_abs = float(side_abs.sum())
    if wall not in by_strike.index or total_abs <= 0:
        return {"score": 0, "verdict": "LOW CONFIDENCE", "class": "regime-neg"}

    wall_abs = float(abs(by_strike.loc[wall, side_col]))
    concentration = wall_abs / total_abs
    sorted_abs = side_abs.sort_values(ascending=False)
    second_abs = float(sorted_abs.iloc[1]) if len(sorted_abs) > 1 else 0.0
    dominance = wall_abs / max(second_abs, 1.0)
    distance_pct = abs(float(wall) - spot) / max(spot, 1.0)
    oi_total = float(by_strike[oi_col].sum())
    oi_share = float(by_strike.loc[wall, oi_col]) / max(oi_total, 1.0)

    concentration_score = 35.0 * min(concentration / 0.25, 1.0)
    dominance_score = 25.0 * min(dominance / 2.0, 1.0)
    proximity_score = max(0.0, 20.0 - min(20.0, distance_pct * 700.0))
    oi_score = 15.0 * min(oi_share / 0.20, 1.0)
    time_score = 5.0 if hours_remaining >= 0.25 else 2.0
    score = concentration_score + dominance_score + proximity_score + oi_score + time_score

    broken = (side == "call" and spot > float(wall)) or (side == "put" and spot < float(wall))
    if broken:
        score -= 20.0

    score = int(max(0, min(100, round(score))))
    if broken:
        verdict, cls = "ALREADY BREACHED", "regime-neg"
    elif score >= 70:
        verdict, cls = "HOLD LIKELY", "regime-blue"
    elif score >= 45:
        verdict, cls = "TEST / RESPECT", "regime-orange"
    ...
    return {
        ...
        "oi_share": round(oi_share * 100, 1),
    }


def compute_0dte_walls(ticker_obj, ticker_symbol, display_label, display_spot, strike_scale):
    T_0dte, hours_remaining, today_str = time_to_0dte_expiry_years()
    expiries = list(getattr(ticker_obj, "options", []) or [])
    if today_str not in expiries:
        return {
            "available": False,
            "reason": f"No {today_str} 0DTE chain returned for {ticker_symbol}.",
            "expiry": today_str,
        }

    try:
        chain = ticker_obj.option_chain(today_str)
    except Exception as exc:
        return {
            "available": False,
            "reason": f"Failed to fetch {ticker_symbol} 0DTE chain: {exc}",
            "expiry": today_str,
        }

    calls = _clean_0dte_side(getattr(chain, "calls", pd.DataFrame()))
    puts = _clean_0dte_side(getattr(chain, "puts", pd.DataFrame()))
    if calls.empty or puts.empty:
        return {
            "available": False,
            "reason": f"0DTE chain for {ticker_symbol} had no usable open interest.",
            "expiry": today_str,
        }
    if display_spot <= 0 or strike_scale <= 0:
        return {
            "available": False,
            "reason": "Invalid spot conversion for 0DTE walls.",
            "expiry": today_str,
        }

    calls["strike"] = (calls["strike"] * strike_scale).round(1)
    puts["strike"] = (puts["strike"] * strike_scale).round(1)
    atm_iv_0dte = get_0dte_atm_iv(calls, puts, display_spot)

    calls["call_oi"] = calls["openInterest"]
    puts["put_oi"] = puts["openInterest"]
    calls["call_gamma"] = calls["strike"].apply(
        lambda K: bs_gamma(display_spot, K, T_0dte, RISK_FREE_RATE, atm_iv_0dte)
    )
    puts["put_gamma"] = puts["strike"].apply(
        lambda K: bs_gamma(display_spot, K, T_0dte, RISK_FREE_RATE, atm_iv_0dte)
    )
    calls["call_gex"] = calls["call_gamma"] * calls["call_oi"] * EQ_MULTIPLIER * display_spot**2
    puts["put_gex"] = -puts["put_gamma"] * puts["put_oi"] * EQ_MULTIPLIER * display_spot**2

    call_by = calls.groupby("strike", sort=True)[["call_oi", "call_gex"]].sum()
    put_by = puts.groupby("strike", sort=True)[["put_oi", "put_gex"]].sum()
    by_strike = call_by.join(put_by, how="outer").fillna(0.0).sort_index()
    for pct in [0.15, 0.20, 0.30, 0.50, 1.0]:
        window = by_strike[
            (by_strike.index >= display_spot * (1 - pct)) &
            (by_strike.index <= display_spot * (1 + pct))
        ]
        if len(window) >= 8:
            by_strike = window
    ...
    if call_gex.abs().sum() <= 0 or put_gex.abs().sum() <= 0:
        return {
            "available": False,
            "reason": f"0DTE GEX was zero after filtering {ticker_symbol}.",
            "expiry": today_str,
        }
    ...
    net_0dte_gex = float(by_strike["call_gex"].sum() + by_strike["put_gex"].sum())

    call_score = score_0dte_wall_strength(by_strike, call_wall, "call", display_spot, hours_remaining)
    put_score = score_0dte_wall_strength(by_strike, put_wall, "put", display_spot, hours_remaining)

    return {
        "available": True,
        "label": f"{display_label} 0DTE",
        ...
        "net_gex_b": round(net_0dte_gex / 1e9, 3),
        "call_score": call_score,
        "put_score": put_score,
        "strike_count": int(len(by_strike)),
    }


def odte_backquant_block(label, odte):
    now = datetime.now().strftime("%d/%m/%Y, %H:%M:%S")
    if not odte or not odte.get("available"):
        reason = odte.get("reason", "0DTE chain unavailable.") if odte else "0DTE chain unavailable."
        expiry = odte.get("expiry", "n/a") if odte else "n/a"
        return (
            f"0DTE GEX Walls [{label}] - {now}\n"
            f"Expiry checked: {expiry}\n"
            f"Status: UNAVAILABLE\n"
            f"Reason: {reason}\n"
        )

    call = odte["call_score"]
    put = odte["put_score"]
    return (
        f"0DTE GEX Walls [{label}] - {now}\n"
        f"Expiry: {odte['expiry']} | Hours to close: {odte['hours_remaining']:.2f} | "
        f"0DTE ATM IV: {odte['atm_iv'] * 100:.1f}%\n"
        f"Method: same GEX wall convention as aggregate engine; 0DTE chain only; T from hours to 16:00 ET.\n"
        f"---\n"
        f"Call Wall Resistance: ${odte['call_wall']:,.1f} - {call['verdict']} "
        f"({call['score']}% confidence, {call['concentration']}% side GEX concentration)\n"
        f"Put Wall Support: ${odte['put_wall']:,.1f} - {put['verdict']} "
        f"({put['score']}% confidence, {put['concentration']}% side GEX concentration)\n"
        f"Net 0DTE GEX: ${odte['net_gex_b']}B | Usable strikes: {odte['strike_count']}\n"
        f"Note: confidence is level strength / hold-risk scoring, not a guarantee.\n"
    )
...
if run:
    ...
    st.markdown("**0DTE BackQuant Paste Block:**")
    st.code(odte_backquant_block(label, item.get("odte", {})), language=None)
