...
def build_scaled_index(symbol, index_symbol, fallback_scale, override, override_min,
    ...
    try:
        auto_converted_spot = first_valid_close(index_symbol)
        conversion_source = "INDEX"
    except Exception:
        auto_converted_spot = None
        conversion_source = "CFD_OVERRIDE" if has_override else "MISSING"

    if has_override:
        display_spot = float(override)
        conversion_source = "CFD_OVERRIDE"
    elif auto_converted_spot is not None:
        display_spot = float(auto_converted_spot)
    else:
        raise RuntimeError(
            f"{display_label} converted spot unavailable. Enter your live {display_label} CFD spot override."
        )
    ...
    return {
        ...
        "auto_index": float(auto_converted_spot) if auto_converted_spot is not None else display_spot,
...
def render_level_card(lvl):
    ...
    warnings_html = "".join(
        f"<div class=\"level-row\" style=\"padding:3px 0;\"><span class=\"level-label\">Warning</span>"
...
<div style="background:#0d1117;border:1px solid #1a2a3a;border-radius:8px;padding:10px 16px 6px 16px;
  ...
  CFD SPOT OVERRIDES &nbsp;-&nbsp; leave at 0 to use live converted index prices, or enter your broker CFD spot
</div>
""", unsafe_allow_html=True)

ov_col1, ov_col2 = st.columns(2)
with ov_col1:
    us500_override = st.number_input("US500 CFD spot", min_value=0.0, value=0.0, step=0.1, format="%.1f")
with ov_col2:
    us100_override = st.number_input("US100 CFD spot", min_value=0.0, value=0.0, step=0.1, format="%.1f")
...
with op_col1:
    us500_open = st.number_input("US500 open price", min_value=0.0, value=0.0, step=0.1, format="%.1f")
with op_col2:
    us100_open = st.number_input("US100 open price", min_value=0.0, value=0.0, step=0.1, format="%.1f")
...
if run:
    ...
    s = data["spy"]
    q = data["qqq"]
    s_lvl, s_open_iv, s_iv_pct = recompute_levels("us500", s["filt"], s["index_spot"], us500_open, s["atm_iv"], dte(s["expiry"]), "US500")
    q_lvl, q_open_iv, q_iv_pct = recompute_levels("us100", q["filt"], q["index_spot"], us100_open, q["atm_iv"], dte(q["expiry"]), "US100")

    st.success(f"Levels ready - {datetime.now().strftime('%d %b %Y %H:%M')}")
    tab1, tab2 = st.tabs(["US500", "US100"])

    with tab1:
        override_tag = " override" if us500_override > 1000 else ""
        auto_note = f" auto converted: {s['auto_index']:.1f}" if us500_override > 1000 else ""
        st.markdown(f'<span class="source-badge badge-src">US500 converted spot {s["index_spot"]:.1f}{override_tag}{auto_note}</span>', unsafe_allow_html=True)
        iv_dir = "down" if s_iv_pct < 0 else "up"
        st.markdown(f'ATM IV: **{s["atm_iv"]*100:.1f}%** | Open IV: **{s_open_iv*100:.1f}%** | IV Change: **{iv_dir} {abs(s_iv_pct):.1f}%**')
    ...
    with tab2:
        ...
        auto_note = f" auto converted: {q['auto_index']:.1f}" if us100_override > 5000 else ""
        st.markdown(f'<span class="source-badge badge-src">US100 converted spot {q["index_spot"]:.1f}{override_tag}{auto_note}</span>', unsafe_allow_html=True)
        iv_dir = "down" if q_iv_pct < 0 else "up"
        st.markdown(f'ATM IV: **{q["atm_iv"]*100:.1f}%** | Open IV: **{q_open_iv*100:.1f}%** | IV Change: **{iv_dir} {abs(q_iv_pct):.1f}%**')
