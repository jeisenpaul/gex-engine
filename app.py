...
def levels(df, spot, label="", days=30, iv_change=0.0, price_change=0.0):
    ...
    # Clamp to filtered range — max pain landing above the call wall or below the
    # put wall is a data artefact from far-OTM OI noise. Pin it to the nearest
    # filtered strike within the tradeable band.
    _filt_lo = float(filt["strike"].min())
    _filt_hi = float(filt["strike"].max())
    max_pain = float(np.clip(raw_max_pain, _filt_lo, _filt_hi))
