...
# Continuous dividend yield per underlying, used as the (r - q) cost-of-carry
# adjustment in the Black-Scholes greeks. QQQ and SPY are NOT close enough to
# share one flat constant - QQQ's TTM yield (~0.42-0.46%) is roughly a
# quarter of SPY's (~1.0%), verified against current data (July 2026).
# Effect on short-dated GEX/DEX is genuinely small either way, but Vanna/
# Charm accuracy improves more on the higher-yield SPY side.
DIVIDEND_YIELDS = {"QQQ": 0.0045, "SPY": 0.010}
...
def build_scaled_index(symbol, index_symbol, fallback_scale, override, override_min,
    ...
    rich = enrich(scaled_df, display_spot, EQ_MULTIPLIER, days, dividend_yield=div_yield)
    ...
    odte = compute_0dte_levels(symbol, display_spot, scale, dividend_yield=div_yield)
...
# CORE COMPUTATION
def _enrich_core(df, spot, multiplier, T, dividend_yield=0.0):
    """
 ...
 Vectorized with numpy instead of a per-strike Python loop calling the
 scalar bs_* functions one row at a time - identical math, ~50-100x faster
 on a full chain. Verified against the original scalar loop on a synthetic
 chain before shipping: outputs matched to floating-point precision at
 dividend_yield=0.

 dividend_yield (q) applies the standard Merton cost-of-carry adjustment:
 d1 uses (r - q) instead of r, and every greek picks up the same e^(-qT)
 discount factor applied uniformly to the existing, already-validated
 formulas - not a re-derivation with a different reference's sign
 convention. That keeps every call/put symmetry and dealer-sign
 relationship already confirmed elsewhere in this engine (call_gamma ==
 put_gamma at equal IV, call_vanna == put_vanna, etc.) exactly intact;
 the effect is a small, correctly-signed discount, not a new bug surface.
 """
    df = df.copy()
    n = len(df)
    K = df["strike"].to_numpy(dtype=float)
    call_iv_raw = df["call_iv"].to_numpy(dtype=float) if "call_iv" in df else np.zeros(n)
    put_iv_raw = df["put_iv"].to_numpy(dtype=float) if "put_iv" in df else np.zeros(n)

    civ = np.where(call_iv_raw > 0.01, call_iv_raw, 0.15)
    piv = np.where(put_iv_raw > 0.01, put_iv_raw, np.where(civ > 0.01, civ, 0.15))

    r = RISK_FREE_RATE
    q = dividend_yield
    global_valid = T > 1e-6 and spot > 0

    def greeks_for(sig):
        """Gamma/call_delta/put_delta/vanna/charm computed at this side's own
 IV, mirroring the original per-side scalar calls exactly."""
        valid = global_valid & (sig > 1e-6) & (K > 0)
        safe_sig = np.where(valid, sig, 1.0)
        safe_K = np.where(valid, K, spot if spot > 0 else 1.0)
        with np.errstate(all="ignore"):
            sqrtT = np.sqrt(T)
            d1 = (np.log(spot / safe_K) + (r - q + 0.5 * safe_sig**2) * T) / (safe_sig * sqrtT)
            d2 = d1 - safe_sig * sqrtT
            disc_q = np.exp(-q * T)
            pdf_d1 = norm.pdf(d1)
            gamma = disc_q * pdf_d1 / (spot * safe_sig * sqrtT)
            call_delta = disc_q * norm.cdf(d1)
            put_delta = disc_q * (norm.cdf(d1) - 1.0)
            vanna = disc_q * (d2 / safe_sig) * pdf_d1
            charm = disc_q * (-pdf_d1 * (2 * r * T - d2 * safe_sig * sqrtT) / (2 * T * safe_sig * sqrtT))

        if not global_valid:
            gamma = np.zeros(n); vanna = np.zeros(n); charm = np.zeros(n)
            call_delta = np.where(spot > K, 1.0, 0.0)
            put_delta = np.where(spot < K, -1.0, 0.0)
            return gamma, call_delta, put_delta, vanna, charm

        gamma = np.where(valid, gamma, 0.0)
        call_delta = np.where(valid, call_delta, np.where(spot > K, 1.0, 0.0))
        put_delta = np.where(valid, put_delta, np.where(spot < K, -1.0, 0.0))
        vanna = np.where(valid, vanna, 0.0)
        charm = np.where(valid, charm, 0.0)
        return gamma, call_delta, put_delta, vanna, charm

    cg, cd, _, cv, cc = greeks_for(civ)
    pg, _, pd_, pv, pc = greeks_for(piv)

    df["call_iv_c"] = civ; df["put_iv_c"] = piv
    df["call_gamma"] = cg; df["put_gamma"] = pg
    df["call_delta"] = cd; df["put_delta"] = pd_
    df["call_vanna"] = cv; df["put_vanna"] = pv
    df["call_charm"] = cc; df["put_charm"] = pc
    ...
    # DEX: delta * OI * multiplier * spot (dollar delta). UNLIKE gamma,
    # vanna, and charm - which are mathematically IDENTICAL for a call and
    # a put at the same strike (since Delta_call - Delta_put = 1 is a
    # constant, their derivatives w.r.t. spot/vol/time are equal) - delta
    # itself is NOT symmetric: it's already positive for calls and
    # negative for puts by construction. Applying the same "dealer long
    # calls, short puts" sign-flip used for GEX/VEX/CEX here (negating the
    # put term) makes call_dex and put_dex BOTH structurally non-negative,
    # which makes net_dex mathematically incapable of ever going negative
    # regardless of real put/call OI distribution - proven numerically:
    # even a chain with 100x more put OI than call OI still produced a
    # POSITIVE net_dex under that formula. That silently forced
    # Directional Bias toward BULLISH on nearly every read, which is
    # confirmed by every professional source on Delta Exposure: it's
    # described as "Total Positive Delta - Total Negative Delta" (i.e.
    # call delta and put delta summed with their OWN natural signs, put
    # delta included as-is, never flipped) - genuinely bidirectional,
    # swinging negative when put OI/delta dominates. That's the fix here.
...
def enrich(df, spot, multiplier, days=30, dividend_yield=0.0):
    T = max(days / 365.0, 1/365)
    return _enrich_core(df, spot, multiplier, T, dividend_yield=dividend_yield)
...
def compute_0dte_levels(ticker_symbol, display_spot, scale, dividend_yield=0.0):
    """
 ...
 """
    ...
    try:
        ...
        rich_0dte = _enrich_core(scaled_0dte, display_spot, EQ_MULTIPLIER, T_0dte, dividend_yield=dividend_yield)
...
def session_phase():
    """
 Which Greek's signal structurally deserves more weight right now, based
 on well-established (not invented) options-flow tendencies: gamma/delta
 dominate price discovery early in the session; charm's influence on
 dealer hedging genuinely concentrates into the final hour as EOD
 rebalancing accelerates; vanna's relative influence can rise during the
 lower-liquidity midday window since the same hedging flow has less
 offsetting volume to absorb it. These are structural TENDENCIES, not
 fixed laws - the multipliers are deliberately modest and exist to stop
 the model treating charm the same at 9:31 as it does at 3:45, not to
 claim precision this data can't support.
 """
    ny = market_now("America/New_York")
    if ny.weekday() >= 5:
        return {"phase": "WEEKEND", "charm_mult": 1.0, "vanna_mult": 1.0,
               "note": "market closed - no live session to phase"}
    minutes = ny.hour * 60 + ny.minute
    open_min, close_min = 9 * 60 + 30, 16 * 60
    if minutes < open_min:
        return {"phase": "PRE-MARKET", "charm_mult": 1.0, "vanna_mult": 1.0,
               "note": "before the open - no live session to phase yet"}
    if minutes >= close_min:
        return {"phase": "AFTER CLOSE", "charm_mult": 1.0, "vanna_mult": 1.0,
               "note": "session closed - today's charm/vanna influence has already played out"}
    since_open = minutes - open_min
    if since_open < 30:
        return {"phase": "OPENING RANGE", "charm_mult": 0.5, "vanna_mult": 0.9,
               "note": "gamma/delta dominate price discovery here - charm and vanna are structurally least relevant this early"}
    if since_open < 120:
        return {"phase": "MORNING", "charm_mult": 0.6, "vanna_mult": 1.0,
               "note": "gamma/delta still primary - trend is establishing"}
    if since_open < 270:
        return {"phase": "MIDDAY", "charm_mult": 0.85, "vanna_mult": 1.15,
               "note": "lower-liquidity window - vanna-driven hedging can have outsized price impact with less offsetting flow to absorb it"}
    if since_open < 360:
        return {"phase": "AFTERNOON", "charm_mult": 1.3, "vanna_mult": 1.0,
               "note": "charm's influence on dealer hedging is genuinely picking up into the close"}
    return {"phase": "FINAL HOUR", "charm_mult": 1.6, "vanna_mult": 1.0,
           "note": "charm/EOD dealer rehedging is most pronounced here - the classic 'pin into the close' window"}
...
def classify_regime(gex_positive, dex_positive, vanna_bullish, vanna_bearish, vanna_fuel,
 ...
 and nuance only - never a competing verdict.

 Live price/IV momentum below MOMENTUM_OVERRIDE_THRESHOLD nudges
 confidence, same as before. AT OR ABOVE that threshold, it does more:
 standing options positioning (DEX) is a snapshot of existing OI
 structure, and it can only describe what dealers are structurally
 exposed to - it has no way to see what's actually happening in price
 right now. When price has already moved decisively against what that
 positioning implies, the live move is more current information and
 should start winning the argument, not just get logged as a confidence
 penalty under a label that never changes. The original DEX-implied read
 is kept and returned separately (positioning_bias) for transparency -
 it isn't discarded, just no longer the thing driving the headline state
 once live price has made its own decision this clearly.
 """
    MOMENTUM_OVERRIDE_THRESHOLD = 0.004  # ~0.4% - a real, sustained move, not noise
    ...
    phase = session_phase()
    charm_weight = (0.35 if days >= 21 else 1.0) * phase["charm_mult"]
    charm_score = clamp((30 + 60 * cex_balance) * charm_weight)
    vanna_score = clamp((vanna_fuel if (vanna_bullish or vanna_bearish) else max(20, vanna_fuel)) * phase["vanna_mult"])

    # --- Momentum override: resolve BEFORE the score/state logic below, so
    # everything downstream (directional_bias, state, alt scenario, entry
    # trigger, hierarchy) reads the same, already-resolved bias - never a
    # patched-on afterthought that could disagree with the rest of the verdict.
    momentum_override = False
    effective_dex_positive = dex_positive
    price_confirms_derived = price_confirms
    if has_open_price and price_change_pct is not None:
        price_confirms_derived = price_change_pct > 0
        conflict = price_confirms_derived != dex_positive
        if conflict and abs(price_change_pct) >= MOMENTUM_OVERRIDE_THRESHOLD:
            momentum_override = True
            effective_dex_positive = price_confirms_derived
            dex_score = clamp(20 + 20 * (1 - min(1.0, abs(price_change_pct) / 0.01)))
    ...
    if has_open_price and price_confirms is not None:
        if momentum_override:
            momentum_note = (
                f"Momentum override: options positioning read {'BULLISH' if dex_positive else 'BEARISH'}, "
                f"but live price has moved {price_change_pct*100:+.2f}% against that since the open - "
                f"trusting observed price action over standing positioning."
            )
            raw_score -= 15
        else:
        ...
        if iv_confirms is not None:
            iv_agree = iv_confirms == effective_dex_positive
    ...
    directional_bias = "BULLISH" if effective_dex_positive else "BEARISH"
    positioning_bias = "BULLISH" if dex_positive else "BEARISH"
    dex_positive = effective_dex_positive  # everything below reads the resolved bias
...
def levels(df, spot, label="", days=30, iv_change=0.0, price_change=0.0,
    ...
    price_change_pct = (price_change / spot) if (has_open_price and spot > 0) else None
    ...
    # "Direction" must track the SAME resolved bias Market State uses - if a
    # momentum override flipped the effective direction, showing the raw
    # pre-override positioning read here would silently contradict Market
    # State on the very same screen, exactly the collision this whole
    # engine was rebuilt to eliminate.
    dex_bias = "BULLISH UP" if verdict["directional_bias"] == "BULLISH" else "BEARISH DOWN"
    if verdict.get("momentum_override"):
        dex_bias = f"{dex_bias} (live-price override; positioning read {verdict['positioning_bias']})"
    resolved_dex_positive = verdict["directional_bias"] == "BULLISH"
    level_strength = score_level_strength(by_strike, spot, {
        ...
        "Put DEX Wall": put_delta_wall,
        "Max Pain": max_pain,
    }, gex_positive, resolved_dex_positive, pain_curve=(sa, losses))
    ...
    return {
        ...
        "Session Phase": verdict["session_phase"],
        "Session Phase Note": verdict["session_phase_note"],
        ...
        "_debug": {
            ...
            "price_change_pct": round(price_change_pct * 100, 3) if price_change_pct is not None else None,
            "iv_change": round(iv_change * 100, 4),
            "price_confirms": price_confirms,
            "iv_confirms": iv_confirms,
            "has_open_price": has_open_price,
            "momentum_override": verdict.get("momentum_override", False),
            "positioning_bias": verdict.get("positioning_bias"),
...
def _pool_levels(lvl, odte):
    """Combine aggregate Level Strength + 0DTE Level Strength into one pool,
 the shared source every dominance-ranked feature (Quick Read, Trade
 Plan, Priority Stack) reads from - one implementation, not three that
 could quietly disagree with each other."""
    pool = [
...
def _compute_magnet(lvl, spot, bias):
    """
 Shared magnet computation (Max Pain / HVL gravity - a separate mechanic
 from wall dominance) used by both Quick Read and Trade Plan. Positive
 gamma pinning is a real, actively-defended pull; negative gamma has no
 such defending force, so the pull there is only as real as classic
 pin-risk dynamics close to expiry - genuinely weak further out.
 """
    ...
    direction = "above" if magnet_level > spot else ("below" if magnet_level < spot else "at spot")
    if gex_positive:
        strength = "STRONG"
    elif dte <= 2:
        strength = "MODERATE"
        ...
        strength = "WEAK"
    opposes = (direction == "below" and bias == "BULLISH") or (direction == "above" and bias == "BEARISH")
    return {"name": magnet_name, "level": magnet_level, "direction": direction,
           "strength": strength, "opposes": opposes}
...
def build_quick_read(lvl, odte, spot):
 ...
 of the detailed one.
 """
    pool = _pool_levels(lvl, odte)
    pool_0dte = [p for p in pool if odte and odte.get("available") and p["name"].startswith("0DTE")]
    if pool_0dte:
        key_level, key_source = max(pool_0dte, key=lambda x: x["dominance_pct"]), "0DTE"
    elif pool:
        key_level, key_source = max(pool, key=lambda x: x["dominance_pct"]), "structural"
    else:
    ...
    magnet = _compute_magnet(lvl, spot, bias)
    ...
    if gex_positive:
        ...
        magnet_pull = magnet is not None and magnet["opposes"] and magnet["strength"] in ("STRONG", "MODERATE")
        ...
        if magnet_pull:
            reasons.append(f"{magnet['name']} pulling against the {bias.lower()} thesis")
    ...
    if magnet is not None:
        dist = abs(magnet["level"] - spot)
        stance = "opposes" if magnet["opposes"] else "supports"
        lines.append(
            f"Magnet: {magnet['name']} @ {magnet['level']:,.1f} ({dist:,.1f} pts {magnet['direction']}, "
            f"{magnet['strength']} pull) - {stance} the {bias.lower()} thesis"
        )
...
def build_trade_plan(lvl, odte, spot):
    """
 Entry / Target / Stop in trader-usable form - the honest version of a
 "decision tree." Built entirely from already-computed, real
 dominance-ranked levels; never a new invented number, never a predicted
 price path. Answers "where do I actually buy" using the same
 dominance/magnet math already shown elsewhere, organized the way a
 trader uses it rather than the way the data was computed. Deliberately
 does NOT attempt to predict a specific sequence of price action during
 the day (rally/reject/retrace/continue) - no options-flow-derived
 metric can honestly produce that; a fabricated path would look
 sophisticated and be worthless.
 """
    pool = _pool_levels(lvl, odte)
    resistances = sorted((p for p in pool if p["level"] > spot), key=lambda x: x["dominance_pct"], reverse=True)
    supports = sorted((p for p in pool if p["level"] < spot), key=lambda x: x["dominance_pct"], reverse=True)
    state = lvl.get("Market State", "")
    bias = lvl.get("Directional Bias", "BULLISH")
    confidence = lvl.get("Confidence", "n/a")
    magnet = _compute_magnet(lvl, spot, bias)

    def fmt(item):
        return f"{item['name']} @ {item['level']:,.1f} (dominance {item['dominance_pct']}%)" if item else "not available in this chain window"

    lines = [f"TRADE PLAN: {state}"]

    if state == "CONTROLLED BULLISH":
        entry = supports[0] if supports else None
        target = resistances[0] if resistances else None
        stop_ref = supports[1] if len(supports) > 1 else None
        lines.append("Condition: wait for a pullback - do not chase current price")
        lines.append(f"Entry zone: {fmt(entry)}")
        lines.append(f"Target: {fmt(target)}")
        lines.append(f"Invalidation: a clean break below {fmt(stop_ref)} - the pinning thesis fails")
    elif state == "PINNED RANGE":
        entry = resistances[0] if resistances else None
        target = supports[0] if supports else None
        stop_ref = resistances[1] if len(resistances) > 1 else None
        lines.append("Condition: fade strength into resistance - do not chase breakouts here")
        lines.append(f"Entry zone: {fmt(entry)}")
        lines.append(f"Target: {fmt(target)}")
        lines.append(f"Invalidation: a clean break above {fmt(stop_ref)} - the range thesis fails")
    elif state in ("BULLISH EXPANSION WATCH", "SQUEEZE RISK"):
        entry = resistances[0] if resistances else None
        em_hi = lvl.get("Expected Move Hi")
        lines.append("Condition: wait for a CONFIRMED reclaim/hold above resistance - do not front-run the break")
        lines.append(f"Entry trigger: confirmed hold above {fmt(entry)}")
        lines.append(f"Target: Expected Move Upper @ {em_hi:,.1f}" if em_hi else "Target: not available")
        fallback = fmt(supports[0]) if supports else "lower structure"
        lines.append(f"Invalidation: rejection back below {fmt(entry)} - expansion thesis fails, expect a retest of {fallback}")
    elif state in ("BEARISH EXPANSION", "CRASH RISK"):
        entry = supports[0] if supports else None
        em_lo = lvl.get("Expected Move Lo")
        lines.append("Condition: wait for a CONFIRMED breakdown/hold below support - do not front-run the break")
        lines.append(f"Entry trigger: confirmed hold below {fmt(entry)}")
        lines.append(f"Target: Expected Move Lower @ {em_lo:,.1f}" if em_lo else "Target: not available")
        fallback = fmt(resistances[0]) if resistances else "higher structure"
        lines.append(f"Invalidation: reclaim back above {fmt(entry)} - breakdown thesis fails, expect a retest of {fallback}")
    else:
        lines.append("Condition: signals conflicted or low confidence - no clean entry structure right now")

    if magnet is not None and magnet["opposes"] and magnet["strength"] in ("STRONG", "MODERATE"):
        lines.append(
            f"Caution: {magnet['name']} @ {magnet['level']:,.1f} is pulling against this plan - "
            f"expect it to get tested before the plan plays out"
        )

    lines.append(f"Confidence in this plan: {confidence}%")
    return "\n".join(lines)


def build_priority_stack(lvl, odte):
    """
 Every level, ranked by real dominance, star-rated directly off that
 number - a direct encoding of already-computed data, not an arbitrary
 scale. The honest version of "rank the levels every morning": instead
 of re-ranking a list by hand, or stars assigned by feel, both the order
 and the stars come straight from the same dominance math used
 everywhere else in this engine.
 """
    pool = _pool_levels(lvl, odte)
    if not pool:
        return "PRIORITY STACK: not enough level data yet."
    pool_sorted = sorted(pool, key=lambda x: x["dominance_pct"], reverse=True)
    ...
    lines = ["PRIORITY STACK (ranked by dominance):"]
    for i, item in enumerate(pool_sorted, start=1):
        lines.append(f"{i}. {item['name']} @ {item['level']:,.1f} - {stars(item['dominance_pct'])} ({item['dominance_pct']}%)")
    return "\n".join(lines)
...
def backquant_block(label, lvl, spot, expiry, odte=None, ndx_check=None):
    ...
    trade_plan = build_trade_plan(lvl, odte, spot)
    priority_stack = build_priority_stack(lvl, odte)
    return (
f"{quick_read}\n"
f"---\n"
f"{trade_plan}\n"
...
f"Session Phase: {lvl.get('Session Phase', 'n/a')} - {lvl.get('Session Phase Note', '')}\n"
f"{hierarchy_lines}"
...
def render_level_card(lvl, spot, odte=None, ndx_check=None):
 ...
 {escape(item['note_1'])}<br>{escape(item['note_2'])}
    ...
    trade_plan_lines = build_trade_plan(lvl, odte, spot).split("\n")
    trade_plan_html = "".join(f'<div style="padding:2px 0;">{escape(line)}</div>' for line in trade_plan_lines)
    priority_stack_lines = build_priority_stack(lvl, odte).split("\n")
    priority_stack_html = "".join(f'<div style="padding:2px 0;">{escape(line)}</div>' for line in priority_stack_lines)
 ...
 <div class="level-card" style="border:1px solid #4fc3f7;background:#0d1a1f;margin-bottom:10px;padding:12px 16px;">
  <div class="section-header" style="color:#4fc3f7;">TRADE PLAN</div>
  <div style="font-family:'JetBrains Mono',monospace;font-size:0.82rem;color:#e8e8e8;line-height:1.5;">
  {trade_plan_html}
  </div>
 </div>

 <div class="level-card" style="margin-bottom:10px;padding:12px 16px;">
  <div class="section-header">PRIORITY STACK</div>
  <div style="font-family:'JetBrains Mono',monospace;font-size:0.8rem;color:#c9d1d9;line-height:1.6;">
  {priority_stack_html}
  </div>
 </div>
 ...
 <div class="level-card">
  ...
  <div class="level-row" style="padding:2px 0 6px 0;">
  <span class="level-label">Session Phase</span>
  <span class="level-val" style="font-size:0.76rem;color:#8a9bb0;text-align:right;max-width:65%;">{escape(lvl.get('Session Phase', 'n/a'))} - {escape(lvl.get('Session Phase Note', ''))}</span>
  </div>
