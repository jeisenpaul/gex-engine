...
def build_scaled_index(symbol, index_symbol, fallback_scale, override, override_min,
    ...
    odte = compute_0dte_levels(ticker, symbol, display_spot, scale)
...
def compute_0dte_levels(ticker_obj, ticker_symbol, display_spot, scale):
    """
 ...
 instead of the main chain's DTE.
 """
    try:
        now_ny = market_now("America/New_York")
        today_str = now_ny.strftime("%Y-%m-%d")
        available_expiries = list(with_retry(lambda: getattr(ticker_obj, "options", []) or []))
        ...
        raw_0dte = option_chain_frame(ticker_obj, today_str, ticker_symbol)
        scaled_0dte = raw_0dte.copy()
        ...
        return {
            "available": False,
            "reason": f"0DTE fetch failed for {ticker_symbol}: {e}",
        }
...
def score_0dte_strength(odte, aggregate_lvl, spot):
 ...
 score_level_strength() above, built from signals specific to a same-day
 chain rather than reusing the aggregate scorer's exposure columns directly:

 - Dominance (0-35): this strike's share of total same-side 0DTE gamma in
 ...
 - Liquidity (0-15): OI actually sitting at the wall strike. Thin/zero
 ...
 - Aggregate confluence (0-35): how close the 0DTE wall sits to the
   structural engine's Call Wall / Put Wall / GEX Flip / HVL. This is the
   single highest-conviction signal - when the intraday wall and the
   structural wall line up, that is the highest-confidence level on the
   board; when they diverge, the 0DTE wall is flying solo for the session.
 """
    ...
    for name, level, dominance, oi_at_wall in sides:
        dominance_score = min(35.0, dominance * 120.0)
        liquidity_score = min(15.0, (oi_at_wall / 50.0) * 15.0)
        ...
        confluence_score = max(0.0, 35.0 - min(35.0, agg_dist_pct * 700.0))
