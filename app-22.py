...
def levels(df, spot, label="", days=30, iv_change=0.0, price_change=0.0, filter_pct=0.15):
    ...
    #
    # Case A — user entered NO session open price at all (open_price == 0).
    ...
    #   The iv_change == 0.0 alone is NOT a reliable guard — it can be legitimately
    #   0 on the first refresh of a real session (open_iv anchored to current_iv).
    #
    # Case B — price_change is exactly 0 AND iv_change is exactly 0.
    ...
    # But if price_change != 0, the user DID enter an open price — never suppress.
    #
    # Use STRICT inequalities for price_up / iv_falling so that zero is neutral:
    price_up_strict   = price_change > 0     # strictly up — zero is NOT bullish
    iv_falling_strict = iv_change    < 0     # strictly falling — zero is NOT bullish
    ...
    return {
        ...
        # ── DEBUG STATE (always populated for diagnostics) ────────────────────
        "_debug": {
...
def render_debug_panel(lvl):
    ...
    no_input_warn = "⚠️ no_live_input=True → Tier 1/2 SUPPRESSED" if d.get("no_live_input") else "✅ live inputs active"
...
bull_signals = <b style="color:#{'00e676' if d.get('bull_signals',0)==3 else ('ffd740' if d.get('bull_signals',0)==2 else '#c9d1d9')}">{d.get('bull_signals', 0)}</b> / 3  →  Tier {'1 🔥' if d.get('bull_signals')==3 else ('2' if d.get('bull_signals')==2 else '3 (fallback)')}<br>
bear_signals = <b style="color:#{'ff5252' if d.get('bear_signals',0)==3 else ('ffd740' if d.get('bear_signals',0)==2 else '#c9d1d9')}">{d.get('bear_signals', 0)}</b> / 3  →  Tier {'1 🔥' if d.get('bear_signals')==3 else ('2' if d.get('bear_signals')==2 else '3 (fallback)')}<br><br>
...
if run:
    ...
    with tab1:
        ...
        st.markdown(f'<span class="source-badge badge-src">US500 {s["index_spot"]:.1f}{override_tag}  (SPY×{s["scale"]:.4f}){auto_note}</span>',
                    unsafe_allow_html=True)
    ...
    with tab2:
        ...
        st.markdown(f'<span class="source-badge badge-src">US100 {q["index_spot"]:.1f}{override_tag}  (QQQ×{q["scale"]:.4f}){auto_note}</span>',
                    unsafe_allow_html=True)
